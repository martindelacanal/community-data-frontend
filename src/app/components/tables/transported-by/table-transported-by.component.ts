...
@Component({
  selector: 'app-table-transported-by',
  templateUrl: './table-transported-by.component.html',
  styleUrls: ['./table-transported-by.component.scss']
})

export class TableTransportedByComponent implements OnInit, AfterViewInit {

  dataTransportedByTable: transportedByTable;
  dataSource: any;
  ...
  ngOnInit() {
    ...
    this.getDataTransportedByTable(this.filterForm.value);
  ...
  openDialogEnableDisableElement(id: string, enabled: string): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result.status) {
        this.tablesService.enableDisableElement(id, 'transported_by', enable).pipe(
          finalize(() => {
            this.getDataTransportedByTable(this.filterForm.value);
          })
        ).subscribe({
  ...
  dialogDownloadCsv(): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.status) {
        ...
        this.tablesService.getTransportedByFileCSV(result.data).subscribe({
          next: (res) => {
            const blob = new Blob([res as BlobPart], { type: 'text/csv; charset=utf-8' });
            const url = window.URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'transported-by-table.csv';
  ...
  private getDataTransportedByTable(filters?: any) {
    this.loading = true;
    this.tablesService.getDataTransportedByTable(this.pagina + 1, this.columna, this.ordenarTipo, this.buscarValor, this.translate.currentLang, filters).subscribe({
      next: (res) => {

        this.pagina = res.page;
        this.columna = res.orderBy;
        this.ordenarTipo = res.orderType;
        this.dataTransportedByTable = res;

        this.numOfPages = this.dataTransportedByTable.numOfPages;
        this.totalItems = this.dataTransportedByTable.totalItems;
        this.dataSource = new MatTableDataSource(this.dataTransportedByTable.results);
